...
const Show = ({show}) => {
    return (
        ...
        <Link to={'/show/' + show._id}>
        <Card.Title as='div'>
        <strong>{(show.date).substring(0,10)}</strong>
        <br/>
        <strong>{show.time.hour < 9 ? '0'+show.time.hour : show.time.hour}:{show.time.minutes < 9 ? '0'+show.time.minutes :show.time.minutes}</strong>
        <br/>
